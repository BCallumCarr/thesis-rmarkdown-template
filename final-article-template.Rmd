---
title: "A (Relatively) No-Nonsense RMarkdown Thesis Template"
author: "Bradley C. Carruthers"
date: "August 28, 2019"
...
```
<!--
```{r wordcount, eval=FALSE, echo=FALSE, cache=FALSE}
...
systemcall <- paste("system('C:\\Users\\Brad\\Downloads\\TeXcount_3_1\\texcount.pl -inc -incbib -total -sum ", noftex, "', intern=TRUE)", sep="") # paste together texcount system command  
...
count <- regmatches(sum.row, regexpr(pattern, sum.row) ) # extract digits
...
This is a template designed for a dissertation/thesis, or maybe even a fancy essay. It is a work in progress, requires knowledge of RMarkdown as well as Latex, and it would help if you knew how to navigate a .tex file to fix some niggly issues when knitting this. Please contact me if you have any suggestions/comments.

The layout of the paper is as follows: Section \ref{Lit} gives a brief overview of relevant literature; section \ref{Data} presents the model and describes certain aspects of the data descriptively and graphically; \ref{OLS} is a section, section \ref{Recom} makes some recommendations for further research and section \ref{Concl} concludes. 
...
## Empirical Model: Education Production Function \label{Model} 
...
You can reference using bibtex like this: @Shepherd2011 notes that the education production form in equation \ref{eq:EP1} entails relatively strict assumptions regarding the dynamics of education and initial endowment measures. 
...
Table \ref{tab:Desc} in the Appendix displays summary statistics. Table \ref{tab:Dep} below is not in the Appendix.
...
knitr::include_graphics("./Results/Graphs/CumAchieve.png")
```
\centering
...
Reference the figure in-text like this: figure \ref{fig:1} implies something.
...
Note that the references need to be re-ordered in the .tex file manually (at least until I fix it).
